			...
			<el-row class="right-component-item" :gutter="4" type="flex" align='middle' style="border-top: solid 1px #DCDEE3;">
				...
				<el-col :span="16">
					<el-select v-model="dataConfig.type" placeholder="请选择" size="small" style="width: 100%;" @change="datasourceChange">
						<el-option v-for="item in dataSourceType" :key="item.value" :label="item.label" :value="item.value"  v-if="item.visible">
						</el-option>
					</el-select>
				</el-col>
			</el-row>
			<component :is="currentView" :json-data="content" class="right-component-item" style="flex:1;" :api="dataConfig.api" :db-config="dataConfig.dbConfig" @data-change="handleDataChange" @filechange="handleFileChange">
...
<script>
	...
	import DataBaseDataV2 from '../base/DataBaseDataV2.vue'
...
//	const FTO = "to";
	...
	export default {
		name: "DatasourceV2",
		props: props,
		message: "数据设置组件，2018-08-27以后的版本",
		data() {
			return {
				isRight: false,
				isState: true,
		...
		methods: {
				...
				for(var i = 0; i < len; i++) {
					for(var j = 0; j < fieldsLen; j++) {
						if(fields[j].required) {
							var name = fields[j].target ? fields[j].target : fields[j].source;
			...
			validDataBase(bShowDialogResult) {
				...
				if(dbConfig && dbConfig.tableName) {
					var url = this.dbRestUrl + service.DB_SUB_VALIDURI;
					var params = {
