	...
	export default {
		...
		methods: {
			...
			isStateSuccess(data) {
				...
				var name = this.getField(FNAME);
				var lng = this.getField(FLNG);
				var lat = this.getField(FLAT);
				var value = this.getField(FVALUE);
				var bRet = true;
				for(var i = 0; i < len; i++) {
					if(data[i].hasOwnProperty(value)) {
						if(data[i].hasOwnProperty(name) || (data[i].hasOwnProperty(lng) && data[i].hasOwnProperty(lat))) {
							bRet = true;
						} else {
							bRet = false;
							break;
			...
			getData() {
				if(this.dataConfig.type == DataSourceType.DS_STATIC || this.dataConfig.type == DataSourceType.DS_CSVFILE) //静态数据和CSV文件
				{
					...
					return list;
				} else if(this.dataConfig.type == DataSourceType.DS_DATABASE) //数据库模式，目前未处理
				{
					return this.validDataBase();
				} else if(this.dataConfig.type == DataSourceType.DS_API) //API，目前未处理
				{
