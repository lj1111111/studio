	...
	<div class="right-component-sub-title">
		...
		<div v-show="!isRight" class="right-component-item-content">
			<div v-for="(item,index) in seriesData" :key="index" class="right-component-item-splitline-step1">
				...
				<ItemStyle :item-style="item.itemStyle"></ItemStyle>
				<LineStyle :line-style="item.lineStyle"></LineStyle>
				<LabelStyle :label-style="item.label"></LabelStyle>
				<AreaStyle :area-style="item.areaStyle"></AreaStyle>
			</div>
		</div>
	</div>
</template>
...
<script>
	...
	import LineStyle from "./series/LineStyle.vue"
	import AreaStyle from "./series/AreaStyle.vue"
	import { symbolTypes } from "../../constants/Constants.js"
	const props = {
		seriesData: {
			type: Array,
			default: () => {
