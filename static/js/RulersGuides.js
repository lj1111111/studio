...
var RulersGuides = function(evt, dragdrop, childEl, parentEl) {
	'use strict';

	var doc = document.documentElement,
		body = document.body,
		childEle = document.querySelector(childEl),
		parentEle = document.querySelector(parentEl),
		wrapper = null,
		lockHandler = null,
		locked = 1,
		hRuler = null,
		...
		getScrollPos2 = function(el) {
			var t = Math.max(el.scrollTop, el.scrollTop),
				l = Math.max(el.scrollLeft, el.scrollLeft);
...
//			body.appendChild(style);
			setTimeout(function() {
				var rect = getRect(childEl);
				hRuler = new Ruler('h', rect.width);
				vRuler = new Ruler('v', rect.height);
	...
	evt.attach('mousedown', childEle, function(e, src) {
		return false;
		var x = e.offsetX,
			y = e.offsetY,
			guide = null,
	...
	evt.attach('mouseup', childEle, function(e, src) {
		return false;
	...
	evt.attach('keyup', childEle, function(e) {
		return false;
